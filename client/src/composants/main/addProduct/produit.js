...
function AddProduit (){
    ...
    const handlerUpload = async e =>{
        e.preventDefault ()
        try {
            if (!isAdmin) return alert ("Acces non autorisé");
            ...
            setLoading (true);
            const result = await axios.post ('/api/upload', fichier, {
    ...
    const handleSubmit = async e =>{
        ...
        try {
            ...
            if(modif){
                 await axios.put (`/api/produit/${prod._id}`, {...prod, images}, {
                    headers: {Authorization: token}
                })
            } else{
                 await axios.post ('/api/produit', {...prod, images}, {
                    headers: {Authorization: token}
                })
            }
        ...
        <div className="addProduit">
            ...
            <form onSubmit={handleSubmit}>
                <div className="row">
                    ...
                    <input type="text" name="produit_id" id="produit_id" placeholder="produit_id" aria-label="produit_id" required value={prod.produit_id} onChange={handleChangeInput} disabled={prod._id} />
                </div>

                <div className="row">
                    <label htmlFor="libelle">Libelle</label>
                    <input type="text" name="libelle" id="libelle" placeholder="libelle" aria-label="libelle" required value={prod.libelle} onChange={handleChangeInput} />
                </div>

                <div className="row">
                    <label htmlFor="prix">Prix</label>
                    <input type="number" name="prix" id="prix" placeholder="prix" required value={prod.prix} onChange={handleChangeInput} />
                </div>
                ...
                <button type="submit" className="submit">{modif? "Modif" : "Ajout"}</button>
            </form>
